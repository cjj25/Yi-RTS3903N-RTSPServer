...
enum RS_PIXEL_FORMAT_E {

	PIXEL_FORMAT_RGB_1BPP = RTS_PIX_FMT_1BPP,
	PIXEL_FORMAT_RGB_1555 = RTS_PIX_FMT_ARGB1555,
};

struct rts_osd_bitmap_s {
	enum RS_PIXEL_FORMAT_E pixel_fmt;
	uint32_t u32Width;
	uint32_t u32Height;
	void *pData;
};
 ...
 *
 * */
int rts_osd_add_font_lib(void *p, const char *filename,
			 struct rts_osd_lib_info *pinfo);
int rts_osd_rm_font_lib(void *p, int index);

int rts_osd_show_text(void *p, uint8_t blkidx, struct rts_osd_text_t *ptext);
int rts_osd_show_picture(void *p, uint8_t blkidx,
			 int picidx) __attribute__ ((deprecated));
...
int rts_osd_get_font_size(void *p, uint8_t *p_osd_char_w,
			  uint8_t *p_osd_char_h);

int rts_osd_set_bitmap(void *p, uint8_t blkidx,
		       const struct rts_osd_bitmap_s *pstBitmap);
